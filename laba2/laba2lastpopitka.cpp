...
BaseObject* addObjects(BaseObject* obj1, BaseObject* obj2) {
    // Проверка типов объектов и выполнение соответствующих действий
    std::cout << "Debug: obj1 type: " << typeid(*obj1).name() << std::endl;
    std::cout << "Debug: obj2 type: " << typeid(*obj2).name() << std::endl;
    if (dynamic_cast<T1*>(obj1) != nullptr && dynamic_cast<T1*>(obj2) != nullptr) {
        T1* result = Factory::createT1("Результат", static_cast<T1*>(obj1)->value + static_cast<T1*>(obj2)->value);
        return result;
    }
    else if (dynamic_cast<T2*>(obj1) != nullptr && dynamic_cast<T2*>(obj2) != nullptr) {
        std::cerr << "Сложение объектов T2 пока не поддерживается." << std::endl;
        return nullptr;
    }
    else {
